/* Mi Inventario — lógica de la app (sin dependencias).
   Los datos de cada suscriptor se guardan en el servidor (API /api/data). Todos los montos están en USD. */
(() => {
  'use strict';
  ...
  async function flush() {
    ...
    saving = true;
    try {
      await Common.api('PUT', '/api/data', state);
    } catch (e) {
      dirty = true;
      if (e.status === 401) location.href = 'index.html';
      else if (e.status === 402 || e.status === 403) location.href = 'suscripcion.html';
      else toast('No se pudo guardar. Revisa tu conexión y vuelve a intentar.');
    } finally {
      saving = false;
    }
  }
  ...
  (async function boot() {
    try {
      const me = await Common.api('GET', '/api/me');
      const u = me.user;
      if (u.access !== 'active') { location.href = 'suscripcion.html'; return; }
      $('#user-email').textContent = u.email;
      $('#admin-link').hidden = u.role !== 'admin';
      if (u.role !== 'admin') {
        const days = Math.ceil((u.subscriptionEnd - Date.now()) / DAY);
        const end = $('#sub-end');
        end.textContent = days <= 5 ? `Vence en ${Math.max(days, 0)} ${days === 1 ? 'día' : 'días'}` : 'Vence el ' + new Date(u.subscriptionEnd).toLocaleDateString('es', { dateStyle: 'medium' });
        end.classList.toggle('soon', days <= 5);
      }
      const d = await Common.api('GET', '/api/data');
      state = { products: d.products || [], sales: d.sales || [] };
    } catch (e) {
      if (e.status === 401) location.href = 'index.html';
      else if (e.status === 402 || e.status === 403) location.href = 'suscripcion.html';
